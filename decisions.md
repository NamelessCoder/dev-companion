...
## 2026-08-01 — The order a task starts in is one file, and the reading comes last in it
...
## 2026-07-31 — Forward evidence comes from a review, not from a prompt that knows the answer
...
## 2026-07-30 — The component index is curated; its contract comes from the installation
...
- **Decided since:** when `targetVersion` is the active installation and its
  ...
  version it read.
- **Still decided:** derivation does not replace curation. Names, summaries,
...
- **Why compiled CSS:** Composer installations ship
...
- **Would falsify it:** component state that is generated only at runtime and
  appears in neither the compiled CSS nor installed JavaScript, or a styleguide
  template whose first matching example is page scaffolding rather than
  component markup. The former needs another installed source; the latter needs
  an explicit selector in the curated index rather than a more permissive
  extractor.

## 2026-07-31 — A supported range is a property of the package, not of the checkout
...
- **Not decided:** the catalogs keep filtering by a single version. Their
