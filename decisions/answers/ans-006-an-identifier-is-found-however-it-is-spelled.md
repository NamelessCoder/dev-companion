...
- Six shapes reported from two checkouts by
  `feedback/2026-07-31-194504-…` and by
  `feedback/2026-07-31-172753-…`: `ext_tables.php`, `ext_emconf.php`,
...
- The multi-word queries those sessions actually typed still reach nothing, and
  ...
  `"list_type" reaches 1`, `"sc_options" reaches 1`, `"mod.web_layout" reaches
  1`, which is the next step it could not print while those terms reached
  nothing either.
...
- Only a term that carries a separator is compared that way. A query without
  one behaves exactly as it did, and nothing this matched before matches less.
...
- Removing the separators from the haystack too is what makes the comparison
  ...
  separator, so the cost is bounded by how rarely such a term is also a
