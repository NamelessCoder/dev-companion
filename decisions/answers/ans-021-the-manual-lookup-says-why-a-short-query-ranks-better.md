...
Built as the second candidate, and the first is not built beside it. What
decided it is this entry's own **Assumed**: a description a client defers
reaches nobody, while the answer is read by every caller that gets one — its
counter-evidence says the first candidate would have reached one caller, not
that it reaches the next.

Every search result names what it was matched on, as the stems that were
searched for and the field each was found in, and the answer says once that page
