...
reference is, and the lookup does not carry it.
...
- The book it should have returned exists and is versioned like the others.
  ...
  root, and one page per ViewHelper below it — `typo3fluid/fluid/latest/If.html`
  for this one.
...
- The anchors at that root turn out to be navigation rather than a page list, so
  indexing the book adds a request and no candidates.
- Indexing it makes the other answers worse. It is a large book of short titles,
  `FIELD_WEIGHTS` weighs `manual` at 2, and a question that merely says Fluid
