...
The third **Wrong if** fired before the floor was written, measured over the
live index this lookup builds. The four numbers reproduce: two queries clear the
...
queries reach one page and the floor leaves one, because coverage is the share
of the query's weight a page carries and that page carries one word of each of
the other two. So a floor that returns them is at most a third, and a floor that
