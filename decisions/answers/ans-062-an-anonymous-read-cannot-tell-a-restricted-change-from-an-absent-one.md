...
The first **Assumed** was measured and half of it is wrong: asked of the review
server directly, both handles and a change number that exists nowhere all answer
with an empty list. So the tool is consistent after all, and what the report saw
once was the server not answering that once rather than a second shape of the
same effect. The feedback inferred a rule from a single reading, which is what
one reading supports.
