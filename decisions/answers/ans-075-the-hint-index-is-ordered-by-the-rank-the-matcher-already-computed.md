...
ids were reachable, and what it cannot say is what the sessions that do use the
index would lose.

Answered on 2026-08-18 by the maintainer, and **Decided** is reversed on the id
path: a call that names an id and matches one carries the index empty and counts
what was left out, while a call matching by paths or task is untouched. Measured
after the change on one id, the answer falls from 8,227 characters of text to
1,565 and from 10,859 of payload to 1,843, with 93 neighbours counted rather
than listed. The **Wrong if** is a session handed the count, needing one of
those 93, and not asking.
