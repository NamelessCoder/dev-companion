...
- The knowledge was never withheld. `D-AUD-2` decided the resources are not
...
- The `provenance` of a topic no longer decides whether the topic is offered.
  It says what an answer is worth outside the core, which is R-SCO-6, and that
...
- An agent offered a tool it cannot usefully follow does better than one
  ...
  correction beats a gap, and that R-SCO-1's per-path audience is what keeps
