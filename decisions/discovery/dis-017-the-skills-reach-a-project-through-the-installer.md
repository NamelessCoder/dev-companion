...
- The blocker the question opened with is gone. `${CLAUDE_PROJECT_DIR}` resolves
  ...
  `vendor/bin/typo3-dev-companion` and vendor nothing. `archive` is a source type
  of its own, a zip over HTTPS with an optional `sha256` pin. Read from the
  plugins reference on 2026-08-08.
- **Skills alone** needs no transformation at all: `Installer::publishSkill()`
  already writes `<skill>/SKILL.md` with `references/base.md` beside it, which is
  the shape a plugin loads.
- **Skills and the project's own server** works exactly where `install` already
  works — the project required this package — and installs cleanly with the tools
  silently absent where it did not.
- **Skills and a vendored server** is 6.0 MB across 983 files, measured on
  ...
  of a `vendor/` built here, and it ships `bin/cli`, `feedback/` and `scenarios/`
...
- A manifest is static JSON and which entry a project needs is a property of the
  ...
  start.
- Auto-update is off for third-party marketplaces by default, so the update stays
  a user act either way — which is what `R-DIS-025` already prompts for.
...
- Somebody copies `skills/` by hand because `install` did not reach their client,
  which is the failure the plugin would have prevented and `D-SKL-036` declines
  to support.
- A marketplace becomes how Claude Code users find TYPO3 tooling at all, and this
