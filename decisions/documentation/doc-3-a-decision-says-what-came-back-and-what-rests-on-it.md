...
# D-DOC-3 — A decision says what came back, and a requirement says what it rests on
...
- The twelve entries carrying `corrected`, read on 2026-08-02. On three it
  meant reversed and replaced — D-AUD-2, D-KNW-3, D-SCO-8. On three the **Wrong
  if** had fired with nothing replacing it — D-FBK-3, D-KNW-1, D-CAT-2. On
  three one named part was wrong and the rest held; D-FBK-5 says so outright:
  "the order this entry is mostly about … is untouched … what was wrong is the
  number beside it". On D-DIS-3 the **Wrong if** had explicitly *not* fired.
- A decision that was confirmed and later revoked could not be written down.
  D-KNW-3 was confirmed by a run on the morning of 2026-08-02 and revoked by
...
- Twelve entries name a test in prose and nothing read it. One of the names in
  `D-KNW-3` was already stale — a `KnowledgeTest` method renamed that morning —
  ...
  own first draft.
- `R-FBK-7` rests on `D-FBK-5`, which is revoked. The requirement is `held`,
...
- The 56 entries survived being rewritten by script — bullets to sections,
  ...
  made a span unbreakable, which `D-DOC-1` is the entry against and which
...
- An entry appears whose outcome is none of the three — most likely one where
  ...
  `D-SCO-1`'s shape and was filed under `corrected` because there was nowhere
  else.
