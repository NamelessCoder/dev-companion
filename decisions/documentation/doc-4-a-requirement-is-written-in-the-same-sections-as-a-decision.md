...
# D-DOC-4 — A requirement is written in the same sections as a decision
...
`D-DOC-3` gave decisions sections because a label repeated up to seven times in
...
- Requirements carry a median of two labelled fields and a maximum of two —
  ...
  twice in one file. Nothing like the repetition `D-DOC-3` was written against.
- `Held by` names more than one test on 60 of the 123 entries, three or more on
  34, and nine on `R-AUD-4`, all inside one comma-separated sentence with the
