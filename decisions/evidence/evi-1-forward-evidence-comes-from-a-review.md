...
`README` and `Documentation/` that three runs walked past is `R-PRJ-6`.
`REVIEW-02` run 4 answered the open feedback that no extension review reaches
the console and settled `D-EVI-3`; run 5 produced four feedbacks, all archived
...
those carry weight already: `D-EVI-2` rests on `SKILL-07` read by hand, and
