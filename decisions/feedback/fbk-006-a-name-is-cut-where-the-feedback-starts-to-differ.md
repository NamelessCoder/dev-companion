...
- 25 of 56 open feedback carried one of three names, 17 of them
  ...
  have separated them. The eight feedback filed in thirteen seconds that
  morning were about Extbase, pid semantics, functional-test databases, cache
  clearing and four other things.
...
- On a collision, read the first line of the feedback that already carry the
  ...
  a feedback that has nothing else to say. What this rejects is asking the
  agent for a title: one more field in a tool whose parameters are already its
  documentation, and the field a session fills last is the one it fills worst —
  the observation is what it came to write.

## Assumed

- That the first difference is the part worth naming. Where a series is
  numbered — "5. Recommended: add en.xlf files" — the number leads and the name
...
- Feedback start colliding on a slug without sharing an opening at all — two
  different sentences cut to the same 48 characters — in which case skipping
  the shared words changes nothing and the counter is doing the work again. Or
  a series turns out to differ only in its middle, so the first difference is a
