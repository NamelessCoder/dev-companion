...
The fourth thing a priority could say is gone, on the day it was written: a card
written for a feedback starts at `low`, which is where absence put it, said in a
word.
...
an optional field cannot be required — so the check had to stay silent about the
one case it most wanted to report. The first **Wrong if** is now the whole of
the watch.
