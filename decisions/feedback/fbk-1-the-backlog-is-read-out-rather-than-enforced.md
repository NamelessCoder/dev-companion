...
- What the first reading found, on the day it was written. - 105 requirements,
  of which one is `open` and one is `not guarded`. `R-AUD-2` has been open
...
- `bin/cli backlog:list` reads both out, `bin/cli repository:check` closes with
  ...
  All three turn a legitimate state into an error — a principle no test can
  hold and a decision nothing has come back about are not defects — and the
...
- That a session which reads the line acts on it. The item asks for the
  ...
  stays as it is — a much lower bar than fixing it, and the reason the soft
  form was thought sufficient.
- That the 29 standing decisions are mostly standing because they are still
  true, so a count and the oldest is a fair summary. If a third of them turn
  out to have been overtaken, the summary was hiding a queue.

## Wrong if

- The same id is still reported with `no todo names it` after three sessions
  that ran the check, or the standing count only ever grows and no session
  sorts them. Then visibility was not the missing part, the coupling has to
  become a rule, and the shape to reach for is the one rejected here:
...
fourth from the session that merged them — so it does not only ever grow and
...
because they were still true, and the summary was not hiding a queue. Four is a
day rather than a trend, and the oldest standing entry is still older than this
reading.
