...
- **Evidence:** the output on the day it was changed — 62 lines, of which the
  one item at the front was 396 words. Two of that item's three paragraphs were
  history (why it moved ahead on 2026-07-31, what the second run of that day
  produced), and the first imperative sentence was in the third. It also said
  "name it as `E-EXT` **above**", pointing at a section marked `**Not an
  item.**` that `next` does not print, so the instruction resolved to nothing
  for its only intended reader. Of the three standing sections, two were
  performed and one printed a command to run, and nothing in the output
  distinguished them. The listings both exited 0 whatever they found. What
  replaced it is 19 lines and 139 words.
...
- **Wrong if:** sessions start asking for context `next` withheld, or open by
  ...
  cut was in the wrong place. Or a recurring todo blocks the queue for more than
  a session or two, which would mean its command answers "there is work" to a
...
- **Since then** the first **Wrong if** has been measured and did not happen, on
  ...
  [`D-FBK-2`](fbk-2-the-order-of-the-work-is-declared-not-inferred.md): the
