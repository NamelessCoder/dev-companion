...
**`typo3_feedback_record` asks the caller which model is recording the note,
declares the field required, and writes `unknown` rather than refusing the note
when it does not arrive.**

A note that reports behaviour is evidence about a model. Nothing on this side of
the connection knows which one: the MCP handshake carries `clientInfo`, the
client rather than the model behind it, and the SDK keeps it in the session,
...
- **Evidence:** the note of 2026-07-31 17:21 — the conformance skill's
  ...
  judging. What the note could not say is who read it, which is the whole of the
...
- **Decided:** one field, `model`, required in the schema so every client asks
  ...
  unattributed note is visibly unattributed rather than indistinguishable from
...
- **Wrong if:** the recorded notes come back mostly `unknown`, or carrying names
