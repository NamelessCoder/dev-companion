...
- **Decided:** three groups in a fixed order. A cadence in days is an
  ...
  The queue is next, because judging a feedback is what puts an item into it: a
...
- **Assumed:** that the queue empties. It is the mirror image of what this
  ...
  dry starves the feedback exactly as the feedback starved the queue, and the
  directory grows from every session everywhere either way.
- **Assumed:** that oldest-first is the right end to take five from. A feedback that
  has waited while fresher ones arrived in front of it is the one at risk of
  never being read, but the newest feedback is the one about the server as it is
