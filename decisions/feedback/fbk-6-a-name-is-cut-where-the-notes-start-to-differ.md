...
# D-FBK-6 — A name is cut where the notes start to differ

**A note whose slug another note already carries is named from the first word
at which the two observations differ, rather than from a counter or a longer
cut of the same opening.**
...
- **Evidence:** 25 of 56 open notes carried one of three names, 17 of them
  ...
  have separated them. The eight notes filed in thirteen seconds that morning
  were about Extbase, pid semantics, functional-test databases, cache clearing
  and four other things.
- **Decided:** on a collision, read the first line of the notes that already
  carry the name, skip the words they open with in common, and cut the slug
  from what is left. Only the colliding notes are read, so recording still
  costs one directory listing in the ordinary case. The counter stays as the
  fallback for a note that has nothing else to say. What this rejects is asking
...
- **Assumed:** that the first difference is the part worth naming. Where a
  ...
  and the name starts with it; the words after it still say what the note is
  about, so the name is ugly rather than wrong.
- **Assumed:** that the first note of a series keeping the shared opening is
...
- **Wrong if:** notes start colliding on a slug without sharing an opening at
