...
crosses all of them at once — a TCA file, a data processor in TypoScript, a YAML
route enhancer, a Fluid template — and the task text that asks for it names none
of them, so a domain-scoped hint is unreachable exactly when it is needed.
...
- `sitepackage-layout` and `frontend-records` go into `general.json`, whose
  ...
  `Domains::hintCategories()` returns a fixed list, and a category outside it is
  loaded and then filtered away.
...
- A backend-only task comes back with the sitepackage layout because it
  mentioned a content element, or the General category keeps growing until it is
  what every answer is made of. The fix in that case is a category for the
...
to 670 words against a corpus mean of 268, and it describes the package from the
backend that administers it, so a backend query reads well against its body. On
the size, General holds 18 of 65 hints and supplied 35 of 54 matched over the
scenario prompts, with 16 of 29 answers made of it alone.
...
have left the hit standing. The second **Decided** bullet made "content element"
a keyword of Fluid and TypoScript, so both are candidate categories for such a
task however the corpus is filed, and moving `sitepackage-layout` out of General
only moves it into one of them. What answered it was the exclusion already in
`ArchitectureHints::find()`, reached until now by the words "backend module"
alone — though nothing about it was ever about modules. It reads
...
markers win, because a task naming both halves is asking for both — `SITE-05` is
one, and it keeps the layout. `SITE-08` holds the shape that failed.
