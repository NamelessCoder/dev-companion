...
- The model half is answered today. `bin/cli hints:probe` on the feedback's own
  query reaches `project-extension-tests` at `appliesTo(15) + text(79)`, and
...
- That half is stated nowhere and reaches nothing.
  `bin/cli hints:probe "too many test databases left over"` and
  `"live database versus test database"` match no hint at all, and
...
- Not closed on the spot, although this run did read the package. A statement
  ...
  the clause lands is a measurement this run did not make:
...
- That what the harness leaves behind is inside the boundary. `doesNotCover`
  sends "running an installation" to the manual, and the sentence that states
  the model already reaches into DDEV to say which account may create a database
  — so the lifetime of what the test harness creates is read as the harness's
  subject rather than the installation's.
...
- The lifetime is narrower than one clause. A suite that runs against a
  ...
  too strong as phrased.
- The clause is written into `project-extension-tests` and costs the hint the
  queries that reach it today. It is past the ceiling before the clause is
  added, and the sentence that states the model is one of the hint's reachable
  ones.
