...
# D-KNW-048 — What the impexp import rewrites in a site configuration is a gap this server owns
...
shipping nothing but `data.xml` takes. So the corpus does not merely stay
silent here; read on the route the reporting session was on, it points the
other way.
...
- The narrower query reaches nothing. `bin/cli hints:probe "imported site base
  url 404 root"` matches no hint at all and returns 78 candidates as the index.
...
- The manual does not answer it either, which is source 3 of the cost table.
  ...
  — and the base is not among them. Its impact section says configurations
  "can now be preserved across export and import cycles", and it names the base
  URL only as a column of the import preview.
...
- The other route does what the corpus says it does.
  `ImportSiteConfigurationsOnPackageInitialization` copies the shipped
  directory into `config/sites/` and writes back `rootPageId` alone, so a
  `base` shipped in `Initialisation/Site/<identifier>/config.yaml` survives.
...
- Step 1a, and queued rather than closed on the spot. What lands is a statement
  ...
  [`judging.md`](../../documentation/feedback/judging.md) puts on the other
...
- `normal` rather than the `low` the card arrived at. The corpus is not silent
  ...
  installation.
- Not `high`. The correction is one line in `config/sites/<identifier>/
  config.yaml`, the session found it by itself, and nothing is blocked on it.
...
- This entry names the lines rather than leaving the reading to the card. The
  todo is a statement to be written and bound, not an investigation into
...
- That the base rewrite is deliberate and stays. The `@TODO` beside it concerns
  ...
  feedback gives for it — the importer cannot know the target domain — is
  sound.
...
- A session is offered the written statement and still looks for the cause in
  the distribution package. The gap would be in the delivery or the routing,
  and this entry would have answered the expensive rung for a cheap problem.
- The two corpus statements turn out to have been written about the impexp
  route rather than about `Initialisation/Site/`. They would then be plain
  errors rather than sentences a feature outgrew, and step 4 would have covered
  them.
- The sibling `feedback/2026-08-03-162826`, in hand on its own branch today,
  ...
  `--create-site` inert in Composer mode, so no site configuration can carry
  the installation's own URL. The two would then be one gap carried by two
  cards, and one of them should carry both under a `**Serves:**` line naming
  the other.
