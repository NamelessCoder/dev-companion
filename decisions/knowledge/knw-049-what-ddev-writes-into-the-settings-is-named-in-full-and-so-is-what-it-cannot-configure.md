...
5432. Nothing on that path reads `omit_containers` or the driver the
      ...
      written at all is `disable_settings_management`. So DDEV does write the
      `DB` section for a database-less project, as the reporting session
      described, and the one generated file read for the entry was the template
