...
Step 1a landed as two statements, both bound: what the missing attribute does to
the file, and that no schema check reports it.

The third **Wrong if** is settled and holds the other way round than the
feedback read it. The specification makes three attributes required and this one
optional, so the file whose translations are discarded is valid XLIFF, and the
core check the feedback asked for exists — it validates those three and never
reads this one. So the check belongs nowhere, and what a validator cannot see is
stated in the hint instead.

The reach holds without changing a skill. What step 2 would be is established
for whoever finds the first **Wrong if** happening, and it is not queued: the
decision bets on the statement reaching.
