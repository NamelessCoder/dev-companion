...
- `Build/.browserslistrc` is identical on `main`, `14.3`, `13.4` and `12.4` and
  ...
  from the build directory. It therefore decides prefixing and gates nothing —
...
- No per-feature table. Which of anchor positioning, `@starting-style`,
  ...
  after it stopped being one is `HintsTest::noHintStatesSomethingThatOnlyHoldsOnOneBranch`,
  and a table of six is that rule's worst case.
- No tool either. A `lookup` answering "is feature X inside the v14 baseline"
  ...
  — the case [`D-FBK-027`](../feedback/fbk-027-the-server-builds-what-costs-its-caller-round-trips.md)
