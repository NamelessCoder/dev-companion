...
was never missing a form. The wrapped form earns its line for one case only, a
value the join would quote.

What the session paid for is the raw flag, and it is stranger than one attempt
could tell: typing it is what switches, not the value given to it, and its own
...
through it. The third **Wrong if** does not hold — a second project carries the
same variables — so the statement says what those lines mean rather than what a
