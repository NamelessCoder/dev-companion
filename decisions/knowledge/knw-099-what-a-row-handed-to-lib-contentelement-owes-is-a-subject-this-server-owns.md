...
The gap is filled by a hint of its own rather than a statement added to the
neighbour: a caller holding the exception and one asking about template roots
share the entry point and nothing else.

The first **Wrong if** does not hold — the processor is registered on the older
major too, and what the newer ones add is the wiring, which is what the binding
says. The second holds and the statement was written to survive it, stating the
rule as every field the schema declares a capability for and giving the core's
list as what that comes to.

Both **Assumed** hold. The reported failure is a property of the transformation
rather than of that row, and the date fields are the same trap the other way
round. Two failures the judgement did not know about are in the hint, because
both come before the exception a caller would arrive with.
