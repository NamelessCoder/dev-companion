...
event storage, so a package shipping the directory alone has it copied nowhere
and logs nothing. The oldest major is the other side of that boundary.
...
own project is a convention. The packaging half is stated as two lists, one of
them measured and one resting on a source archive nobody measured.
