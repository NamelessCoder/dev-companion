...
- **Corrected on 2026-08-01:** the pairing is off by one release line, and the
  core says so itself. Each covered branch pins the package in its own
  `require-dev` — `^8.3.1` on 12.4, `^9.2.1` on 13.4, `^9.5.0` on 14.3,
  `dev-main` on main — so v12 pairs with 8.x rather than with 7.1.1, and v13
  with 9.x rather than with 8.3.3. Composer resolves a project the same way,
  because a line admits the major it was cut for and the one before it: 8.x
  requires `typo3/cms-core: 12.*.*@dev || 13.*.*@dev` and 9.x `13 || 14`, so an
  extension on v12 installs 8.3.3. The statements are untouched by this: 7.1.1
