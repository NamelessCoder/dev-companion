...
- `R-AUD-1` has named three audiences since 2026-07-29 and the code implemented
...
- The word is `scope`, not `audience`. `audience` stays the idea the repository
  is organised around — `requirements/audience/` and R-AUD-1 through R-AUD-6 —
...
- `outsideCore` is removed from the five output schemas rather than kept beside
  ...
  the scope, and `D-SCO-6` predicted the rename in 2026-07-29. This is a
...
- Answering `project` where the old code answered `outside-core` costs nothing,
  because both are outside the core and R-SCO-2 draws its line there. Eleven of
