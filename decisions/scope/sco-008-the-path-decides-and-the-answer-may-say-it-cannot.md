...
What [`D-SCO-007`](sco-007-the-signals-are-combined-per-call-and-a-call-is-not-a-path.md)
...
- `Build/` is not evidence of the core. The core keeps `Build/Scripts/` and
  ...
  only where the manifest at the root has not already said this is not the
  core. That manifest is what `Instance` reads a checkout's kind from, so
  nothing new reads the disk for it.
...
- The path is read before the call. A path carrying `typo3/sysext/` or an
  ...
  call is consulted only for a path that carries nothing of its own. That is
  the whole of what keeps `META-03` apart, and it is a strict refinement of
...
- `typo3_task_guide` keeps one verdict, because it is asked about one `area`
  and cannot be given two. Naming this a limitation of the tool rather than of
  the decision is deliberate: the audience is per path either way, and what is
  missing is a parameter, not a rule.
- The payload keeps `outsideCore`. `AGENTS.md` has the schemas add fields
  rather than rename them, and a client validating against the declared output
  schema would break on a removal. It now means "the whole call is outside",
  which is what it always was for the calls that had one audience, and
  `audiences` carries the per-path answer beside it. The rename `D-SCO-006`
  predicted happened where `D-SCO-006` located the pull — in the code, where
  every session that touched the flag re-derived a scope sentence from its
  name.
...
- A path with no shape of its own is rare in a call that has one with a shape.
  ...
  undecided one, and the answer is longer for no gain — `composer.json` beside
  a sysext path is the shape of that.
