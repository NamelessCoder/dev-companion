...
- The haystack is `implode(' ', $paths) . ' ' . $text`. After that line nothing
  ...
  draw the distinction even if it wanted to. `D-SCO-3` already draws the
...
- Two things are missing rather than one. The unit of decision has to become
  the path, and the answer has to gain a third value for the case `R-AUD-2`
  names — signals that disagree with nothing to resolve them. A `bool` has room
  for neither, so this is the rename `D-SCO-6` predicted: `outsideCore` becomes
...
- `R-SCO-1`'s ordering survives the change. It orders signals about one piece
