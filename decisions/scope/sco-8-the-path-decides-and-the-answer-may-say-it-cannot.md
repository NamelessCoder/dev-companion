...
- The path is read before the call. A path carrying `typo3/sysext/` or an
  ...
  `R-SCO-1`'s order rather than a change to it: the ranks are the same, and
...
- The payload keeps `outsideCore`. `AGENTS.md` has the schemas add fields
  ...
  predicted happened where `D-SCO-6` located the pull — in the code, where
