...
The second **Wrong if** was tested against an installation session and did not
fire: the symptom it ends on reaches nothing when probed as that session would
phrase it, and reaches two hints on its subject, neither of which states the
thing. So the decision not to touch that workflow holds for the reason it was
made — what an installation session stops on is answered by the changelog at the
major in play, and a line sending the symptom to the hint index would have sent
this one to a miss. What the session reports instead is that nothing invited the
question once the workflow ended, which is `D-SKL-049`'s closing form.
