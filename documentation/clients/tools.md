...
The [readme](../../readme.md) says what each tool is for in prose; these pages
say what goes in and what shape comes back.

- [`typo3_server_scope`](tools/typo3_server_scope.md) — Orientation for this
  server.
- [`typo3_rule_lookup`](tools/typo3_rule_lookup.md) — Search the local TYPO3
  core contribution rules and script notes by topic.
- [`typo3_script_lookup`](tools/typo3_script_lookup.md) — Find notes for TYPO3
  core scripts and commands.
- [`typo3_task_guide`](tools/typo3_task_guide.md) — Build a task checklist
  enriched with matching hints and relevant core checks.
- [`typo3_test_run_guide`](tools/typo3_test_run_guide.md) — Recommend
  Build/Scripts/runTests.sh commands by topic.
- [`typo3_hint_lookup`](tools/typo3_hint_lookup.md) — Return hints for TYPO3
...
- [`typo3_component_lookup`](tools/typo3_component_lookup.md) — Look up TYPO3
  backend UI components by name or topic.
- [`typo3_system_extension_lookup`](tools/typo3_system_extension_lookup.md) —
  Answer whether an extension is part of the TYPO3 core, and on which versions.
- [`typo3_reference_list`](tools/typo3_reference_list.md) — List the worked
  examples the TYPO3 core ships of its own conventions, and what each one is a
  reference for.
- [`typo3_translation_domain_lookup`](tools/typo3_translation_domain_lookup.md)
  — Compute the translation domain an XLF file resolves to, from its path.
- [`typo3_label_lookup`](tools/typo3_label_lookup.md) — Search the labels
  registered in the TYPO3 installation you are working in.
- [`typo3_fluid_namespace_list`](tools/typo3_fluid_namespace_list.md) — List the
...
- [`typo3_configuration_lookup`](tools/typo3_configuration_lookup.md) — Read an
  effective TYPO3_CONF_VARS value from the installation you are working in.
- [`typo3_schema_lookup`](tools/typo3_schema_lookup.md) — List the columns TYPO3
  derives for a table from its TCA.
- [`typo3_backend_module_lookup`](tools/typo3_backend_module_lookup.md) — List
  the backend modules registered in the TYPO3 installation you are working in,
  with the extension that declares each one, its place in the module tree, its
  labels and its route.
- [`typo3_icon_lookup`](tools/typo3_icon_lookup.md) — Validate or find an icon
  identifier in the TYPO3 backend icon registry of the installation you are
  working in.
- [`typo3_changelog_lookup`](tools/typo3_changelog_lookup.md) — Search the TYPO3
  changelog of the installation you are working in.
- [`typo3_project_scope`](tools/typo3_project_scope.md) — Describe the project
  around the TYPO3 installation this server was started in.
- [`typo3_extension_scope`](tools/typo3_extension_scope.md) — Describe what one
  installed extension registers.
- [`typo3_catalog_scope`](tools/typo3_catalog_scope.md) — Report whether
  component contracts come from the active installation or the bundled fallback,
  which TYPO3 core revision the fallback catalogs were taken from, what they
  cover, and how to re-check them.
- [`typo3_commit_message_guide`](tools/typo3_commit_message_guide.md) — Draft
  and check a TYPO3 commit message.
- [`typo3_feedback_record`](tools/typo3_feedback_record.md) — Leave feedback
  about a gap, wrong answer, or missing capability of this knowledge server.
- [`typo3_feedback_list`](tools/typo3_feedback_list.md) — List improvement
  feedback recorded via typo3_feedback_record, newest first, so they can be
  worked off.
