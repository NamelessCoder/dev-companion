...
Tools: typo3_architecture_lookup, typo3_task_guide
Source: knowledge/architecture-hints/ (any)
## Official TYPO3 API, reference and tutorial documentation
Live lookup in the public tables of contents of TYPO3 Explained, TypoScript Explained and the TCA Reference, bound to one covered documentation release. Every result keeps its canonical URL, document, version and section; pass that URL back with the same target version to read the page as text, including headings and code examples. An unreachable service is not an empty search.
...
Tools: typo3_architecture_lookup, typo3_rule_lookup
Source: knowledge/architecture-hints/css.json (any)
...
Source: src/Knowledge/Catalog/TranslationDomain.php, knowledge/architecture-hints/general.json (any)
...
Read from the repository's files — composer.json, package.json, config/sites — so it answers on a fresh clone, before anything is installed or migrated. It describes what is there, and does not judge it. Per extension: the tables its TCA defines and extends, the content elements it adds to tt_content and the Fluid template each renders through, its backend modules and routes, its icons, its site sets, its service tags, its middlewares, its Fluid roots and namespaces, and the shape of its Classes/ directory. Its tables, content elements and icons come from the booted installation where there is one; the rest is declared in files, and where the installation could not be booted the answer says what a file-read list leaves out.
...
## Your checkout: changed files, branch, working tree, local git state
This server answers from its own bundled knowledge and never reads, inspects, or runs anything against a TYPO3 core checkout. It cannot be pointed at one.
...
## The core source itself: API signatures, TCA of a table, existing implementations
The knowledge base describes conventions, not code. No core sources ship with it.
...
## Gerrit server state: review status, patch sets, votes, CI results
The server talks to nothing over the network.
Instead: Use the Gerrit web UI or git. The local side of the workflow — pushing, amending, backporting — is covered by typo3://core/typo3-gerrit-workflow.
...
Instead: Read what the project already has with typo3_project_scope, ask typo3_architecture_lookup with id=site-sets for what a set ships and how its settings resolve against a site's own, and take the configuration format and the language setup from https://docs.typo3.org/.
...
- Working in a concrete file and unsure about the subsystem's conventions → typo3_architecture_lookup
...
```json
{
    ...
    "covers": [
        ...
        {
            "topic": "Core architecture conventions: DI, events and hooks, TCA and FormEngine, FormEngine data providers, DataHandler, routing, Fluid templates and ViewHelpers, frontend page rendering with PAGEVIEW, how a sitepackage is laid out, records in the frontend and the routing for them, registering a content element, Extbase plugins, TypoScript site sets and TSconfig, language files, upgrade wizards, frontend DataProcessors, upgrading an installation, testing strategy, assembling a test suite for a project extension, setting up static analysis for an extension, browser and accessibility tests with Playwright",
            ...
            "source": "knowledge/architecture-hints/",
            "scope": "any"
        },
        {
        ...
        },
        {
            ...
            "source": "knowledge/architecture-hints/css.json",
            "scope": "any"
        },
        {
        ...
        },
        {
            ...
            "source": "src/Knowledge/Catalog/TranslationDomain.php, knowledge/architecture-hints/general.json",
            "scope": "any"
        },
        {
        ...
        },
        {
            ...
            "depth": "Read from the repository's files — composer.json, package.json, config/sites — so it answers on a fresh clone, before anything is installed or migrated. It describes what is there, and does not judge it. Per extension: the tables its TCA defines and extends, the content elements it adds to tt_content and the Fluid template each renders through, its backend modules and routes, its icons, its site sets, its service tags, its middlewares, its Fluid roots and namespaces, and the shape of its Classes/ directory. Its tables, content elements and icons come from the booted installation where there is one; the rest is declared in files, and where the installation could not be booted the answer says what a file-read list leaves out.",
            "tools": [
                "typo3_project_scope",
    ...
    "doesNotCover": [
        ...
        {
            "topic": "Gerrit server state: review status, patch sets, votes, CI results",
            "why": "The server talks to nothing over the network.",
            "instead": "Use the Gerrit web UI or git. The local side of the workflow — pushing, amending, backporting — is covered by typo3://core/typo3-gerrit-workflow."
        },
        {
    ...
    "routing": [
        ...
        },
        {
            ...
            "call": "typo3_architecture_lookup"
        },
        {
