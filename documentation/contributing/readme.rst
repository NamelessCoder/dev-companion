...
* :doc:`working-on-the-server.md <working-on-the-server>` — the checkouts a
  knowledge change is verified against, the environments, the test suite, and
  how the documentation is published.
* :doc:`driving-a-session.md <driving-a-session>` — what has to be true of an
  agent session started from a command line, whether it is carrying a forward
  run or working a todo in a worktree.
* :doc:`writing-a-skill.md <writing-a-skill>` — what a new task skill has to
  show before it exists, the rules it is written under, and what holds each one.
* :doc:`glossary.md <glossary>` — what everything here is called, one line each.
