...
- **Namespace and Tool Naming** reserves `typo3.` for the mandatory part of the
  contract and sends extensions to a prefix of their own.
...
the relative links it already carries resolve onto the reference URIs this
server answers — [`ResourceHandler`](../../src/Sdk/ResourceHandler.php). Move
the scheme and those links resolve nowhere.
...
the rule it gives tool names, or fixing an authority segment such as
...
tool named `typo3_page_read`; on that reading the 26 tools here are unaffected.
It is a reading of one sentence — the draft states the prefix once and never
gives the separator a rule.
