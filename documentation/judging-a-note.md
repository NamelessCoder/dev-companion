...
A note is a session's report about this server, left by an agent working
...
yet. This page is that judgement — what is asked of a note, in which order, on
what evidence, and which of the answers may be given without asking first.

[feedback.md](feedback.md) says where a note lives and what happens to it once
it is worked off. This is the step between the two.

## The one question

Every note gets the same question, and it is not whether the note is right:

**Could anything about this server have prevented it?**

That reframing is the whole of it. Half the notes this server receives are a
session criticising its own work — it did not consider Extbase, it violated the
language-file rules, it never activated the testing skill. Read as self-criticism
those are somebody else's laundry, and they were treated as such. Read as the
question above they are a list of gaps this server could have closed and did
not, which is the most valuable half of the corpus.
...
descent. A note that is not walked down the ladder is guessed at — and the
...
**1a — the knowledge is missing.** `bin/cli hints probe "<the note's own
...
`.checkouts/`, and the manual after it — and writing that. Never the note's own
suggestion copied into `knowledge/`: the note is a report about this server, and
its author was guessing about TYPO3 exactly as much as the judging run would be.
...
Neither needs the note to ask for it, and it usually will not: the session
...
arguments to get an answer. Those are stated in almost every note, because the
debrief prompt asks for them.
...
note carries it: a session that invented the right order itself, and so the next
one may not; a session that went in an order that cost it the task; and the same
sequence arrived at independently by two sessions. That last one is evidence
[writing-a-skill.md](writing-a-skill.md) explicitly says nothing can read off a
file — *that a domain earned a skill at all* — and the notes are where it comes
from. Two core patch reviews in the open corpus ran the same sequence and both
praised it, and a third note proposes the skill outright.

The bar in [writing-a-skill.md](writing-a-skill.md) still has to be cleared; the
notes are what shows it has been reached, not a shortcut past it.

Both become queued verdicts, never ones closed on the spot: a tool and a skill
are contracts, and a skill is installed into somebody else's project, where a
mistake is not corrected by the next release.

**The category is not the verdict.** `tool-gap`, `missing-knowledge`,
`wrong-answer` are how the reporting session saw it from where it stood, with no
view of this repository. A note filed as `missing-knowledge` is regularly a
...
**Evidence:** an entry in `decisions/` whose **Wrong if** this note satisfies.

This is the step that points upward instead of producing work, and the reason it
exists is that a note from practice *is* the event a **Wrong if** describes.
Nothing reads them against each other today: `bin/cli backlog list` names the
oldest standing decision, but not whether one of the open notes contradicts it.

The signal is usually two notes rather than one — the same property reported as
a strength by one session and as a cost by another. Both notes are then
evidence, and neither is wrong.
...
settling a question, and the cheap end answers most notes:
...
| 2 | `.checkouts/` | local | whether what the note claims about TYPO3 holds |
...
Steps 2 to 4 are owed to any note that makes a claim about TYPO3 itself — a
changelog number, a deprecation, a version boundary. Those cannot be settled
from this repository, and a note taken on trust becomes a knowledge entry
...
## The verdict names the gap, not the fix
...
notes. A solution named from that position is recalled, not read — and the todo
that follows copies it down, so the guess ends up in `knowledge/` with a
...
the sighting is back to five notes a session, which is the problem this process
exists to solve.
...
that exists. The change is made in the same run and the note is archived by the
commit that makes it.

Two things put a note on the other side of that line, and either is enough:
...
The note describes a version of this server that no longer exists. Re-run its
query; where today's answer is correct, the note is answered and the commit says
which query was run and what came back, so the judgement can be disagreed with.
...
records the next concrete step, and the note stays open until the commit that
implements it archives it.

### Trimmed — *autonomous*

Part of the note is answered and part is not. The answered part goes, the rest
stays open. Never archive a note that was only half addressed.
...
verdict this process may reach on its own.

A note is discarded by nobody here. What is written instead is the proposal:
...
Step 5. The note is recorded against the decision it bears on, and the question
goes up. What comes back is either a decision that stands with the cost now
written into it, or one that is revised — and both are answers the note earned.

## What a todo may not repeat

A todo that serves a note does not restate it. It carries what the note does
not: the verdict — which step of the ladder, on what evidence — and the next
concrete step. One or two sentences, not a paragraph.

The duplication that exists today is produced by a rule rather than by
sloppiness. [AGENTS.md](../AGENTS.md) asks a todo's paragraph to be written *in
enough detail that someone who has read nothing else can start*, and where the
todo already names the note that describes the problem, that demand can only be
met by copying it. The same account then sits in `feedback/`, in `todo.md` and,
once a requirement is written, a third time in `requirements/` — three places to
write it, three to maintain, and any one of them can be brought up to date on
its own and leave the other two saying something else.

So the demand is met when the todo is **handed over** rather than when it is
written: `bin/cli next` prints the note a todo serves along with it. Nothing is
repeated, the reader still has everything, and the `**Serves:**` line stops
being a reference nobody follows.

Each of the three then carries only its own half, which is what
[feedback.md](feedback.md) already says they are for: the note is what happened
once, the requirement is what must hold from now on, the todo is what to do
next.

## Why every verdict is written immediately

Not at the end of a bundle, at the end of each note.

An honest judgement costs the note itself, usually a probe, a search, sometimes
a checkout read — call it fifteen thousand characters. A debrief of twenty-one
notes therefore does not fit in one context, and it never did. What happened
instead is that a run got through six, ran out of room, and left nothing behind:
the verdicts were in the context and nowhere else, so the next run read the same
six notes again and paid the same cost.

So the context stops being the state. Every verdict is on disk before the next
note is opened, and where a run ends does not matter, because everything before
that point is recorded. This is the same property `feedback/` was given from the
start and for the same reason — `src/Feedback.php` says it plainly: *one note
per file, so concurrent agents never touch the same file*.

The trace is a **journal plus the commit**. One line per note — date, ladder
step, evidence, verdict — and the commit that made the change stays the answer.

The note itself is not written to. Where a note **stands** is what says what
became of it, and a `verdict:` field somebody edits is the one thing that could
disagree with where it is.

The journal is not bookkeeping. It is the only thing that carries any
relationship between notes, which matters because notes are judged one at a time
and no run sees more than one: the run that opens note 40 reads the journal's
forty lines, not the forty notes. That is what catches the two cases a single
note cannot state — one note in the open corpus explicitly corrects three
earlier ones, and the same gap gets reported by several sessions in a row. Both
are invisible per note and obvious in a list of verdicts.
...
argument no individual note can make.

## Why a note is judged in its own context

The ladder is written to be executed, not discussed. Fixed steps, fixed evidence
per step, a fixed shape coming back — because the run that judges a note is not
the run that keeps the overview.

The main run never reads a note. It reads the list, hands each note (or bundle)
to a run with a fresh context, and gets back the few lines of the verdict:
which step, which evidence, which outcome, which diff. It grows by ten lines per
note instead of fifteen thousand characters, and the expensive research is paid
by a context that is thrown away afterwards.
...
## One note per run, in a loop

Nothing is bundled. One note is judged per run, the context is cleared, the next
run starts empty — and that repeats until there is no unjudged note left.
...
is in front of it, writes the verdict, and ends. Where the context would have
filled, a new one starts instead.

What this gives up is reading several notes from one debrief together, and the
journal is what pays for it — see above. What it buys is that the process no
longer has a size limit at all.

So `bin/cli feedback next` hands over **one** note, the oldest with no journal
entry, and exits nonzero while any remain. That is the same shape `bin/cli next`
already has for todos, down to naming the page that says how the thing is
worked.
...
It also recorded, as an assumption, *that the queue empties*, with the note that
nothing had run long enough to show it. The decision was taken on 2026-08-01
against 56 open notes and 38 queued items; later the same day there are 69 open
notes and 33 queued. The queue moved by five, the pile by thirteen.
...
verdict is on disk before the next note is opened, that assumption is gone and
the limit costs more than it protects. The requirement changes with the commit
that builds this; the decision is corrected in place rather than deleted,
because the assumption it made is exactly where the next one is likely to sit.

**`OPEN:` what triggers the loop.** The decision's own suggestion is a cadence
in days, like every other appointment. The alternative is that it is not on a
clock at all: a run that a person starts when the pile is worth a pass, since a
loop that resumes has no deadline to miss.
