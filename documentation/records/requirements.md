...
[what a decision is](decisions.md). Where an entry goes and how it is written is
...
[`bin/cli unresolved:list`](readme.md) reads them out instead, together with
whether a todo names the id. Nothing in `requirements/` reaches the order of the
work on its own; that listing is the whole of the coupling.
