...
* :doc:`answer-sources.md <tools/answer-sources>` — the five sources an answer
  can come from, and what each one needs to be running before it can.
* :doc:`knowledge-base.md <knowledge-base>` — what ships in ``knowledge/``,
  which is where almost every answer is read from.
* :doc:`versions.md <versions>` — how a statement that holds for some TYPO3
  versions and not others says so, and why a catalog withholds an entry instead
  of qualifying it.
* :doc:`asking-the-installation.md <asking-the-installation>` — the order an
  installation's own answers are looked up in, how the probe is delivered, and
  what a fallback owes the caller.
* :doc:`interface-contract.md <interface-contract>` — a community proposal for
  an MCP interface contract for TYPO3, read as a reference: what it says and
  what would change here if it were ever adopted.
