...
Add hints for the image processing pipeline: that FAL is one entry point among several rather than the foundation, that the processing task layer unwraps to a local path via getForLocalProcessing() before any work happens and everything below it operates on path strings, that GraphicalFunctions exposes resize/imageMagickConvert/getImageDimensions on paths, that ContentObjectRenderer::getImgResource() and GIFBUILDER are separate path-based entry points, and that resolving an EXT: resource for f:image routes through the storage-0 fallback which ResourceFactory marks for removal. A session reasoning about what can be changed in this area will otherwise infer necessity from the only call path it happens to read, as I did.
