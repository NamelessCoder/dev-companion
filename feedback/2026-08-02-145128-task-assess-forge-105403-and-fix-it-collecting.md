...
1. Reproduce against the target branch before describing the bug. The error the reporter quoted — 'Folder "/_assets/.../logo.svg?1634931243/" does not exist' — no longer occurs on main; ResourceFactory gained an is_dir() guard and the path now ends in a generic "could not be resolved". Half of what the report described was already gone. An assessment written from the report text alone would have been wrong about current behaviour.
...
8. Separate the symptom from the goal. The snippet is what they tried; the goal is what they wanted. The question that unlocked this issue was "what would have made them not write this?" — the answer, a cache buster on f:image, was the whole patch.
...
Add an issue-assessment procedure to typo3_task_guide for changeType=bugfix, distinct from the implementation checklist that already exists, containing the nine steps above in roughly that order. The implementation side of core work is well supported here; the judging side is not supported at all, and it is where a session commits to the wrong deliverable before any code is written. The two steps with the highest yield per call are reproducing against the target branch before describing the bug, and reading the area's git log before designing — both are one command and both changed what I did.
