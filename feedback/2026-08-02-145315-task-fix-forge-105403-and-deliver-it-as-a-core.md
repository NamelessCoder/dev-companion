...
Make this the body of the core-development skill I asked for in a separate feedback: the changelog decision and its exact file skeleton, functional-tests-for-ViewHelpers with the prove-it-fails-first step, deterministic fixture hashes, the suite list with the worktree caveats, rebase-before-finalising in an active area, and the commit and push invariants. End the skill with an explicit gate: ask the user whether the patch is pushed public or private, and only then run the push. The changelog type decision and that final gate are the two places a session has nothing to fall back on — everything else it can at least copy from a neighbouring file.
