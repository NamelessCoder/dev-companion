...
- typo3-core-patch-development: add the trigger "you are about to change a file in the core checkout" rather than "write a patch and carry it to review". By the time I knew I was writing a patch I was already editing, and the skill's own subject matter (changelog, checks, commit message) only became relevant at the end — too late for me to think of opening it.

Separately: this session began with the server's instruction block telling me to call typo3_project_describe first and typo3_task_guide for the workflow. I called neither. If those two are the intended route into the skills, the instruction block is not achieving it — a client that surfaces skills in the system prompt makes them look like the alternative to the tools rather than the thing the tools hand off to.
