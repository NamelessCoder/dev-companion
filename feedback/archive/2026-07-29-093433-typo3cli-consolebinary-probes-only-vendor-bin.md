...
closed: 2026-07-29
commit: dc47e10
subject: "Read where the installation actually keeps its console"
...
Read config.bin-dir from the root composer.json in consoleBinary() and probe <bin-dir>/typo3 before falling back to vendor/bin/typo3 and bin/typo3. Composer's default is vendor/bin, so honouring the declared value is strictly a superset of today's behaviour and costs one json_decode that requiredPhpVersion() already performs on the same file.
