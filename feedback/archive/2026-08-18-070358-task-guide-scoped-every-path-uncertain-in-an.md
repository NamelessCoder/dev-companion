...
Make an extension key passed as a path resolve to scope "extension" — or, better, resolve scope from the repository the server was started in, which it already knows: root kind "composer-project", a root composer.json of type typo3-cms-extension with extra.typo3/cms.extension-key, and no typo3/sysext/ directory, is unambiguously not a core checkout, and that is decidable without any path argument at all. Where the repository declares its own commands, those are the checks to hand back — typo3_project_describe already reports them with check/change/unknown markings, and task_guide could carry the same list.

Trimmed on 2026-08-18. The second half asked for the core-check suppression to fail closed on scope "uncertain"; that is declined and the reading is in D-SCO-012, which also carries the reproduction of the half above.
