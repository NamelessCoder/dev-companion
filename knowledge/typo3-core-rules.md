...
This file is the first local knowledge base for the TYPO3 CMS MCP server. Keep it practical, versioned, and conservative. Prefer links to official TYPO3 documentation when rules depend on a specific branch or current policy.
...
- Run the narrowest useful test first, then broaden when shared behavior is touched.
...
- Functional tests are expected for persistence, configuration, routing, backend behavior, or integration with TYPO3 services.
- Acceptance or UI tests are useful when the change affects editor or administrator workflows.
- Document tests that could not be executed and why.

## Review Readiness

- The change should be reproducible from the issue or task description.
- The patch should include a concise explanation of the problem and the chosen fix.
