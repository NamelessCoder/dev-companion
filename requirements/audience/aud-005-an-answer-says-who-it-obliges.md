...
**An answer says who it obliges, not only what is true.**

The same sentence can be a condition of a core patch and a convention worth
adopting in a project, and the difference is not visible in the sentence — so it
is data (`scope: "core"`), on the hint where the whole subject is the core's own
...
[R-AUD-001](aud-001-three-audiences-not-one.md) made specific — a core rule
