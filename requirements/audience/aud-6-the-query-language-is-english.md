...
# R-AUD-6 — The query language is English

**The query language is English, and the server says so to the agent rather
than to the user.**

The corpus is written in English and the matching is lexical, so a query in
another language reaches only the words the two happen to share — the
technical loanwords — and otherwise comes back empty. Supporting a second one
