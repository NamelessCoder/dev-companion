...
**From:** a `REVIEW-02` run against `georgringer/news` (2026-07-31) that was
told the extension registers `provider` and `source`; and the measurement that
followed it — on a set-up site with news installed the booted registry holds
1314 icon identifiers against 1289 read from files, the 25 extra being the ones
its `Configuration/Icons.php` builds in a `foreach` plus one TYPO3 derives from
TCA, while a second site without news agreed on all 1287. In the extension's own
...
**Held by:** `Typo3RuntimeTest::theProbeReachesAnInterpreterAndAnswersAsData`,
