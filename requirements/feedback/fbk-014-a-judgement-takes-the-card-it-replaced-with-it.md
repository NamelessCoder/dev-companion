...
another todo's `serves:` arrives after the card was written, and nothing looks
back at it: the card stays in the queue carrying the step every card carries —
judge this feedback — for a judgement that has already been made.
...
to be done. Only reporting is automated: the deletion belongs to the session
that made the judgement, and a command that removes a claimed card is how a
judgement gets lost rather than found.
...
standing beside a judged one — which is why the signal is the step the card
still carries, verbatim, rather than the count.
...
moment the work behind them is done, and both are read by a session that has
read nothing else.
