...
# R-FBK-10 — Work somebody has in hand is offered to nobody else
...
file sits is what it is, and a claim that lived in a field of a file still in the
queue would be a claim only the checkout it was set in can see.
...
The claim carries what tells it from one nobody came back to: the branch the work
is on, so a half-finished diff can be found, and the day it was taken, so a state
that locks everybody else out of one todo can be read as stale.

`bin/cli todo:next` stays where every session starts, worktree or not. A session
told its file name by whoever set it up would be the one session here that begins
differently, and the failure that invites is silent: handed the front of the
