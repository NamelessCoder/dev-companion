...
hint's own statements are where the symptom is written down — «the failure is a
service-not-found at request time» is the sentence a caller arrives with and was
the one thing the matcher could not see. Both are scored, the curated vocabulary
above the prose, so a phrasing somebody anticipated still decides. This does not
withdraw
...
- Measurement itself, both halves of it: the queries that reached nothing before
  ...
  not simply start answering everything.
...
- A sweep is still a sample, so `bin/cli hints:coverage` is what says how much
  of the corpus is reachable at all: which hints their own title does not reach,
