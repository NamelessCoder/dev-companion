...
# R-KNW-3 — A hint says how a subsystem is used
...
to write a datamap. Where a mechanism has a shape that is easy to get wrong —
an order, a naming rule, a step that happens at install time — the hint
states it.

## From

A session that built a site with this server as its only reference and found
the catalog organised around "what must a patch satisfy to be merged" while the
