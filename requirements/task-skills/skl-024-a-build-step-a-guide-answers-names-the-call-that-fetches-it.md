...
to it: what the step does not name is what the session settles for itself, out
of whatever it arrived with. The ids in `typo3_project_describe`'s `guides`
array are delivered at the start of a session, hundreds of calls before the one
that matters becomes relevant, and holding them is not the same as being sent to
one.
