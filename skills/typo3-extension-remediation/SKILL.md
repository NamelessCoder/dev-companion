---
name: typo3-extension-remediation
...
2. Activate `typo3-extension-conformance` and let it run to its report. It owns
   ...
   finding belongs to, and nothing here re-derives any of that. A remediation
   that opens the checkout looking for findings of its own has replaced an audit
   with an impression, and the list it writes is not the one the report would
   have given.
...
## The list is written down and committed before anything changes
...
5. Commit that list on its own, before a single change, with a message from
   `typo3_commit_message_guide` with `workflow="project"`. That commit is what
   the whole order exists for — it is what the maintainer reads to cut items,
   reorder them or stop, and it is what a session that ends halfway leaves
   behind.
6. Do not begin the work in that commit. A list arriving together with the
   changes it produced is one nobody had the chance to disagree with.

## The list is worked off item by item

7. Take the items in the list's order, grouped by the workflow that owns them.
...
8. Activate the owning skill and carry across only the scope and the verified
...
9. Where an item has no owning workflow, it is worked here: the change, the
   project's own check that covers it, and nothing wider than the finding.
10. Commit per item, or per group of items in one owner's area, and mark the
    item's state in the list in that same commit. A list whose state lives in a
    session's memory is what this order was built against.

## What closes it

11. Hand the worked list back to `typo3-extension-conformance` for the re-check.
    It kept responsibility for that when it handed the finding over, and a
    remediation that grades its own work has no evidence the finding is gone.
12. Report what is left: the items still open, the items dropped with what
    dropped them, and every finding the audit reported as open or unassessed
    that this work did not settle. A finished list and an abandoned one read
    alike in a summary.
