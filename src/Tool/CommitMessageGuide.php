...
final class CommitMessageGuide extends ReadOnlyTool
{
    ...
    public static function inputSchema(): array
    {
        return [
            'type' => 'object',
            'properties' => [
                'message' => ['type' => 'string', 'minLength' => 1, 'description' => 'A complete existing commit message to check, subject and trailers included. Unknown trailers such as Change-Id are kept, so an amended patch set stays valid. The exception is workflow="core", which takes Signed-off-by, Co-Authored-By and an agent\'s own session trailer off the draft and says so: a core commit message carries none of them, whatever the checkout you are working in says.'],
