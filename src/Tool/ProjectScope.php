...
final class ProjectScope extends ReadOnlyTool
{
    ...
    {
        return 'Describe the project around the TYPO3 installation this server was started in: its TYPO3 and PHP constraints, the extensions that are its own rather than TYPO3\'s, the sites it configures with the site sets each depends on, and the commands it declares in composer.json and package.json, each with what running it does to the sources: a check that hands the code back as it was, a change that rewrites something, or unknown where the declared body does not say. Read from files only — no console, no database — so it answers on a fresh clone. It also says which environment the repository configures to run itself in — a DDEV project states the PHP its container runs — because that is a different interpreter from the caller\'s shell and the commands below are run in it. Call it before recommending or running a check: the commands listed here are the ones that exist in this repository, the ones marked check are the ones a task told not to change files can run, and where an environment is named they are run inside it rather than directly.';
    }
