...
final class ToolSchemas
{
    ...
    private static function extensionScope(): array
    {
        return self::object([
            ...
            'notReadStatically' => self::listOf(self::string(), 'Registration files that are there but whose entries do not stand in their own text: each assembles its list while it runs, so what it registers is missing from the lists above rather than absent. The booted installation is what answers for them; an empty list here means every file that exists was read.'),
