...
**Serves:** R-AUD-001, R-AUD-005, decisions/

`D-KNW-005` gave the corpus one vocabulary and the enum has five cases, but the
...
become one hint whose statements carry their own — the shape `R-AUD-005` already
...
vocabulary is one: `R-AUD-001` and `D-KNW-005` say so and both are held.
