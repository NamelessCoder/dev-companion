...
nothing here says which page may hold it, so a session seeding a table of its
own has nothing when it picks the pid. Read
`DataHandler::hasPermissionToInsert()` and `isTableAllowedForThisPage()` with
