...
the corpus says where a preview is registered, what it is handed and which
parts not to repeat, and nothing says what belongs in it. Read all nine
templates in
...
`knowledge/hints/content-elements.json`, naming the field kinds a preview
draws from rather than asking for a summary, with a requirement for what has
to keep holding. Settle the version binding while reading: `theme_camino`
ships on 14 only and 13.4 has no worked preview, so the evidence is one major
and the rule may not be.
