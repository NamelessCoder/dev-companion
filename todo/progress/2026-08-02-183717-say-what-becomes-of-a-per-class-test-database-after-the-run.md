...
and `main` for what drops a test database and when, what
`$initializeDatabase = false` and the `pdo_sqlite` files under
`functional-sqlite-dbs/` do to that, and whether the `_ft<7 hex>` suffix is the
whole of what tells a leftover from the live database — that last part is what
the reporting session actually lost. Then measure where the clause fits before
writing it: the sentence it belongs beside is on `project-extension-tests`,
which this run counted at 981 words against the 544-word ceiling
