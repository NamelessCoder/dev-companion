...
`knowledge/hints/fluid.json` and the resource-handling block, marked `since` 15
so 13.4 and 14 answers do not change, and hold one of them with a probe case.
The general question the report raises belongs in the same commit as a sentence,
not a mechanism: where an area was restructured in the target version, the hints
say so themselves instead of routing to `typo3_changelog_lookup`, which a
session under time pressure skips.
