...
priority: normal
waitingOn: >
  somebody starting the subject session by hand, in a current client, who has
  not read this case. Nothing here can drive one. The two `claude` binaries a
  session on this machine can reach are 1.0.65 and 1.0.48, both older than the
  skill mechanism the case measures, and the 2.1.226 the maintainer runs is on
  no `PATH` a session here has. That is why the run below is written as a sheet
  to paste rather than as a step to carry out.
  ...
  still conflicts the way the case needs.
---
