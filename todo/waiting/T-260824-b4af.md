...
priority: normal
waitingOn: >
  whether to propose a core patch to `typo3/AGENTS.md` naming
  `typo3_forge_lookup` beside the `curl` recipe. That file is tracked in the
  core repository, so the correction is a change through Gerrit like any other,
  and what nothing in this checkout decides is whether this server's maintainer
  puts the name of an experimental server into the core's own instructions.
  `documentation/records/judging.rst` says an outward claim is never made
  quietly. The reading behind it is in
