...
priority: normal
waitingOn: >
  whether `typo3_project_describe` volunteers the deprecated-files verdict for
  the extensions that are the project's own, so a session that skips step 2 of
  `skills/base.md` still gets it. That is `documentation/records/judging.rst`
  step 5 — everything was delivered and the design is the price — and it
  changes what an orientation answer hands every caller. The reading behind it
  is in
